...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, 
                      img_states, img_padding_mask, txt_img_relas,
                      cell, initial_state_attention=False, 
                      use_image=True, pointer_image=True,
                      pointer_gen=True, use_coverage=False, prev_coverage=None, cov_type=0):
  """
  Args:
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    if use_image:
        img_attn_len = img_states.get_shape()[1].value
        img_enc_states = tf.concat([img_states, encoder_states], axis=1)
    else:
        img_attn_len = 0
        img_enc_states = encoder_states
    
    img_enc_states = tf.expand_dims(img_enc_states, axis=2)
    ...
    encoder_features = nn_ops.conv2d(img_enc_states, W_h, [1, 1, 1, 1], "SAME") # shape (batch_size,attn_length+196,1,attention_vec_size)
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        def masked_attention(e):
          ...
          if use_image:
              img_enc_padding_mask = tf.concat([img_padding_mask, enc_padding_mask], 1)
          else:
              img_enc_padding_mask = enc_padding_mask
              
          attn_dist *= img_enc_padding_mask # apply mask
        ...
        context_vector = math_ops.reduce_sum(array_ops.reshape(attn_dist, [batch_size, -1, 1, 1]) * img_enc_states, [1, 2]) # shape (batch_size, attn_size).
        context_vector = array_ops.reshape(context_vector, [-1, attn_size])
        
        #print(enc_attn_len)
        if use_image:
            img_attn_dist = attn_dist[:, :img_attn_len]
            enc_attn_dist = attn_dist[:, img_attn_len:]
        else:
            img_attn_dist = None
            enc_attn_dist = attn_dist

      return context_vector, enc_attn_dist, img_attn_dist, coverage
    ...
    if initial_state_attention: # true in decode mode
      ...
      context_vector, enc_attn_dist, img_attn_dist, coverage = attention(initial_state, coverage) # in decode mode, this is what updates the coverage vector
    for i, inp in enumerate(decoder_inputs):
      ...
      if i == 0 and initial_state_attention:  # always true in decode mode
        with variable_scope.variable_scope(variable_scope.get_variable_scope(), reuse=True): # you need this because you've already run the initial attention(...) call
          context_vector, enc_attn_dist, img_attn_dist, _ = attention(state, coverage) # don't allow coverage to update
      else:
        context_vector, enc_attn_dist, img_attn_dist, coverage = attention(state, coverage)
      
      if use_image and pointer_image:
          enc_attn_dist2 = enc_attn_dist + tf.squeeze(tf.matmul(txt_img_relas, tf.expand_dims(img_attn_dist, 2)), 2) 
      else:
          enc_attn_dist2 = enc_attn_dist
      #print(img_attn_len)
      pointer_dists.append(enc_attn_dist2)
      
      if use_image:
          attn_dists.append(tf.concat([img_attn_dist, enc_attn_dist], 1))
      else:
          attn_dists.append(enc_attn_dist)
      
      if use_coverage:
          if cov_type == 0:
              for_cov_dists.append(tf.concat([img_attn_dist, enc_attn_dist], 1))
          elif cov_type == 1:
              for_cov_dists.append(enc_attn_dist)
          elif cov_type == 2:
              for_cov_dists.append(img_attn_dist)
          elif cov_type == 3:
              for_cov_dists.append(enc_attn_dist2)
    ...
    return outputs, state, pointer_dists, for_cov_dists, attn_dists, p_gens, coverage
