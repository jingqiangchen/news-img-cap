...
tf.app.flags.DEFINE_string('vocab_path', common.path_vocab, 'Path expression to text vocabulary file.')
tf.app.flags.DEFINE_string('embedding_path', common.path_embedding, 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_string('exp_name', 'TI-PG', 'Name for experiment. Logs will be saved in a directory with this name, under log_root.')

# Hyperparameters
tf.app.flags.DEFINE_integer('img_fea_num', 196, 'number of image features')
tf.app.flags.DEFINE_integer('img_fea_dim', 512, 'dimension of image features')
...
tf.app.flags.DEFINE_integer('vocab_size', 40004, 'Size of vocabulary. These will be read from the vocabulary file in order. If the vocabulary file contains fewer words than this number, or if this number is set to 0, will take all words in the vocabulary file.')
...
tf.app.flags.DEFINE_boolean('use_image', True, '')
tf.app.flags.DEFINE_boolean('pointer_image', True, '')
tf.app.flags.DEFINE_string('article', "article", '')

# Coverage hyperparameters
tf.app.flags.DEFINE_boolean('coverage', False, 'Use coverage mechanism. Note, the experiments reported in the ACL paper train WITHOUT coverage until converged, and then train for a short phase WITH coverage afterwards. i.e. to reproduce the results in the ACL paper, turn this off for most of training then turn on for a short phase at the end.')
tf.app.flags.DEFINE_float('cov_loss_wt', 1.0, 'Weight of coverage loss (lambda in the paper). If zero, then no incentive to minimize coverage loss.')
tf.app.flags.DEFINE_integer('cov_type', 0, 'type -f coverage. 0: full; 1: text; 2: image')
...
tf.app.flags.DEFINE_integer('ckpt_save_model_secs', 14400, ' ')

tf.app.flags.DEFINE_string('corpus', "corpus", '')
  ...
  with sess_context_manager as sess:
    ...
    while True: # repeats until interrupted
      ...
      if train_step % 100 ==0:
          ...
          if FLAGS.coverage:
              coverage_loss = results['coverage_losses']
...
def main(unused_argv):
  ...
  hparam_list = ['img_fea_num', 'img_fea_dim', 'mode', 
                 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 
                 'max_grad_norm', 'encoder_hidden_dim', 'hidden_dim', 'emb_dim', 'batch_size', 
                 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 'cov_type', 'use_image', 'pointer_gen', 
                 'pointer_image', 'article']
  hps_dict = {}
